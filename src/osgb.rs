...
pub fn osgb_batch_convert(
    ...
    osg_array
        .par_iter()
        .map(|x| if let Err(e) = osgv_convert(
            x.as_str(),
            dir.to_str().unwrap(),
...
fn find_coor_num(str: &str, lvl: usize) -> String {
    ...
    loop {
        if bin_path_buf[start_pos] < b'0' || bin_path_buf[start_pos] > b'9' {
            break;
        }
...
pub fn merge_osgb_tileset(
    ...
    if !path.exists() || !path.is_dir() {
        return Err(From::from(
            format!("path {} does not exists.", path.display()),
        ));
    }
    ...
    for x in path.read_dir().unwrap() {
        ...
        for osg in path.read_dir().unwrap() {
            ...
            if valid_num.len() > 0 {
                ...
                kv_path.entry(lvl_num).or_insert(vec![]).push(
                    entry_osg
                        .path()
                        .to_str()
                        .unwrap()
        ...
        for (lvl, v_path) in kv_path.iter() {
            for path in v_path {
                ...
                let tile = Rc::new(RefCell::new(Tile {
                    boundingVolume: Volume { box_v: box_v.clone() },
                    content: Content {
                        boundingVolume: Volume { box_v: box_v },
                        url: {
                            let v_str = v["root"]["content"]["url"].as_str().unwrap().to_string();
                            format!("Data/{}/{}", stem, v_str)
                        },
                    },
                    geometricError: get_geometric_error(center_y, *lvl as i32),
                    children: vec![],
                }));
